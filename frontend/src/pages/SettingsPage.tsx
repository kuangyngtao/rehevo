...
const CARD_CLASS = `flex h-full min-h-[330px] flex-col rounded-2xl border border-slate-200/80
  bg-white p-5 shadow-sm transition-shadow hover:shadow-md dark:border-slate-800
  dark:bg-slate-900`;

const ICON_WRAP_CLASS = `flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-xl
  bg-primary-50 text-primary-600 dark:bg-primary-900/30 dark:text-primary-300`;

const DETAILS_CLASS = `mb-4 flex-1 space-y-1 rounded-xl border border-slate-100 bg-slate-50/70
  p-3 dark:border-slate-800 dark:bg-slate-950/30`;

const ACTION_BAR_CLASS = `mt-auto flex min-h-12 flex-wrap items-center gap-2 border-t
  border-slate-100 pt-3 dark:border-slate-800`;
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="mb-10">
        ...
        <h1 className="text-4xl font-bold text-slate-950 dark:text-white">系统设置</h1>
        <p className="text-slate-500 dark:text-slate-400 mt-3">统一管理聊天、向量与语音服务，变更前可先完成连接测试。</p>
      </div>
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <motion.button
                  ...
                  className="flex items-center gap-2 px-4 py-2.5 rounded-xl font-medium text-sm
                    bg-primary-600 text-white shadow-md shadow-primary-500/20 hover:bg-primary-700 transition-all"
                >
          ...
          <>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              <motion.div
                ...
                className="w-full max-w-lg rounded-[28px] border border-slate-200/80 bg-white p-7 shadow-2xl dark:border-slate-800 dark:bg-slate-900"
              >
                <h3 className="mb-2 text-xl font-bold tracking-tight text-slate-900 dark:text-white">
                  {editingProvider ? '编辑 Provider' : '新增 Provider'}
                </h3>
                <p className="mb-6 text-sm text-slate-500 dark:text-slate-400">管理模型服务和知识库向量化能力，不会影响已有面试记录。</p>
                ...
                <div className="flex gap-3 justify-end mt-6">
                  <motion.button
                    onClick={closeModal}
                    disabled={saving}
                    className="rounded-xl border border-slate-200 px-5 py-2.5 text-sm font-medium text-slate-600 transition-colors hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                  >
                    取消
                  ...
                  <motion.button
                    ...
                    disabled={saving}
                    className="rounded-xl bg-primary-600 px-5 py-2.5 text-sm font-semibold text-white shadow-md shadow-primary-600/20 transition-colors hover:bg-primary-700 disabled:cursor-not-allowed disabled:opacity-50"
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                  >
                    {saving ? (
          ...
          <>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              <motion.div
                ...
                exit={{ opacity: 0, scale: 0.95, y: 20 }}
                onClick={(e) => e.stopPropagation()}
                className="max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-[28px] border border-slate-200/80 bg-white p-7 shadow-2xl dark:border-slate-800 dark:bg-slate-900"
              >
                <h3 className="mb-2 text-xl font-bold tracking-tight text-slate-900 dark:text-white">
                  {showVoiceModal === 'asr' ? '编辑 ASR 语音识别' : '编辑 TTS 语音合成'}
                </h3>
                <p className="mb-6 text-sm text-slate-500 dark:text-slate-400">调整实时语音链路的连接与输出参数，保存后将在下一次会话生效。</p>
          ...
          <>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              <motion.div
                ...
                exit={{ opacity: 0, scale: 0.95, y: 20 }}
                onClick={(e) => e.stopPropagation()}
                className="w-full max-w-md rounded-[28px] border border-red-100 bg-white p-7 shadow-2xl dark:border-red-950/70 dark:bg-slate-900"
              >
                <h3 className="mb-2 text-xl font-bold tracking-tight text-slate-900 dark:text-white">
                  删除 Provider
                </h3>
                ...
                <div className="flex gap-3 justify-end">
                  ...
                  <motion.button
                    ...
                    disabled={deleting}
                    className="rounded-xl bg-red-600 px-5 py-2.5 text-sm font-semibold text-white shadow-md shadow-red-600/20 transition-colors hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-50"
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                  >
                    {deleting ? (
