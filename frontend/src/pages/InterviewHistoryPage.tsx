  ...
  return (
    <motion.div className="w-full" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      <div className="flex justify-between items-start mb-10 flex-wrap gap-6">
        ...
        <motion.div
          className="flex items-center gap-3 bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-700 rounded-2xl px-4 py-3 min-w-[280px] shadow-sm shadow-slate-200/50 focus-within:border-primary-500 focus-within:ring-2 focus-within:ring-primary-100 dark:focus-within:ring-primary-900/30 transition-all"
          initial={{ opacity: 0, x: 20 }}
          animate={{ opacity: 1, x: 0 }}
        >
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <StatCard icon={TrendingUp} label="平均分数" value={stats.averageScore} suffix="分" color="bg-sky-500" />
        </div>
      )}
      ...
      {!loading && filtered.length === 0 && (
        <motion.div
          className="text-center py-20 bg-white dark:bg-slate-900 rounded-[28px] shadow-[0_18px_45px_-32px_rgba(24,34,53,0.45)] border border-slate-200/80 dark:border-slate-800"
              ...
              <AnimatePresence>
                {filtered.map((item, index) => (
                  <motion.tr
                    ...
                    className="border-b border-slate-100 dark:border-slate-800 hover:bg-primary-50/35 dark:hover:bg-primary-950/20 cursor-pointer transition-colors group"
                  >
