...
export default function Interview({
  ...
  if (isCreating) {
    return (
      <div className="flex items-center justify-center min-h-[50vh]">
        <div className="rounded-[28px] border border-slate-200/80 bg-white px-10 py-9 text-center shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="mx-auto mb-4 h-10 w-10 animate-spin rounded-full border-[3px] border-slate-200 border-t-primary-600 dark:border-slate-700 dark:border-t-primary-400" />
          <p className="font-medium text-slate-700 dark:text-slate-200">正在准备本轮面试...</p>
          <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">正在生成与你的目标匹配的问题</p>
        </div>
      </div>
    );
  }
  ...
  if (error && !session) {
    return (
      <div className="flex items-center justify-center min-h-[50vh]">
        <div className="rounded-[28px] border border-red-100 bg-white px-10 py-9 text-center shadow-sm dark:border-red-950 dark:bg-slate-900">
          <p className="mb-2 font-semibold text-slate-950 dark:text-white">暂时无法开始面试</p>
          <p className="mb-6 text-sm text-red-600 dark:text-red-400">{error}</p>
          <div className="flex justify-center gap-3">
            <button
              onClick={startInterview}
              className="rounded-xl bg-primary-600 px-5 py-2.5 font-semibold text-white shadow-md shadow-primary-600/20 transition-colors hover:bg-primary-700"
            >
              重试
            </button>
            <button
              onClick={onBack}
              className="rounded-xl border border-slate-200 px-5 py-2.5 font-medium text-slate-700 transition-colors hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
            >
              返回
            </button>
          </div>
