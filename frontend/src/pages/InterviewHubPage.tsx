...
export default function InterviewHubPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto">
      {/* 页面标题 */}
      <div className="mb-10">
        <div className="flex items-center gap-2 text-sm font-semibold text-primary-600 dark:text-primary-400 mb-3">
          <Sparkles className="w-4 h-4" />
          Rehevo Practice
        </div>
        <h1 className="text-4xl font-bold text-slate-950 dark:text-white">模拟面试</h1>
        <p className="text-slate-500 dark:text-slate-400 mt-3">选一个方向，在真实对话里找到更自然的表达。</p>
      </div>

      {/* 配置区域 */}
      <div className="bg-white dark:bg-slate-900 rounded-[28px] shadow-[0_18px_45px_-32px_rgba(24,34,53,0.45)] border border-slate-200/80 dark:border-slate-800 p-6 sm:p-8 mb-8">
        <div className="space-y-8">
          {/* 面试模式 */}
          <div>
            <label className="flex items-center gap-2 mb-3 text-sm font-semibold text-slate-800 dark:text-slate-200">
              面试模式
            </label>
              ...
              {([
                ...
                return (
                  <button
                    ...
                    className={`flex items-center gap-3 p-5 rounded-2xl border transition-all duration-200 text-left
                      ${selected
                        ? 'border-primary-500 bg-primary-50/70 dark:bg-primary-950/40 shadow-sm shadow-primary-500/10'
                  ...
                  >
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs leading-5 text-slate-500 dark:text-slate-400">{opt.desc}</p>
                    </div>
                  </button>
                );
              })}
            ...
            {config.loadingSkills ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                {config.skills.map(skill => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex items-center gap-2.5 p-3.5 rounded-2xl border transition-all duration-200 text-left
                        ${selected
                          ? 'border-primary-500 bg-primary-50/70 dark:bg-primary-950/40 shadow-sm shadow-primary-500/10'
                          : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 hover:border-slate-300 dark:hover:border-slate-600 hover:-translate-y-0.5'
                        }`}
                ...
                <button
                  ...
                  className={`flex items-center gap-2.5 p-3.5 rounded-2xl border border-dashed transition-all duration-200 text-left
                    ${config.isCustomSkill
                      ? 'border-primary-500 bg-primary-50/70 dark:bg-primary-950/40 shadow-sm shadow-primary-500/10'
                      : 'border-slate-200 dark:border-slate-700 hover:border-primary-300 dark:hover:border-primary-600 hover:-translate-y-0.5'
                    }`}
                >
          ...
          <AnimatePresence>
            {config.isCustomSkill && (
              ...
              >
                <div className="space-y-3 bg-slate-50 dark:bg-slate-950/50 rounded-2xl p-4 border border-slate-200 dark:border-slate-800">
                  <textarea
              ...
              {DIFFICULTY_OPTIONS.map(opt => {
                ...
                return (
                  <button
                    ...
                    className={`py-3.5 px-4 rounded-2xl border transition-all duration-200 text-center
                      ${selected
                        ? 'border-primary-500 bg-primary-50/70 dark:bg-primary-950/40 shadow-sm shadow-primary-500/10'
                        : 'border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 hover:border-slate-300 dark:hover:border-slate-600 hover:-translate-y-0.5'
                      }`}
          ...
          <button
            ...
            className="w-full flex items-center gap-2 py-3 text-sm text-slate-500 dark:text-slate-400 hover:text-primary-600 dark:hover:text-primary-400 transition-colors"
          >
          ...
          <AnimatePresence>
            {config.showMore && (
              ...
              >
                {/* 简历选择 */}
                <div className="bg-primary-50/70 dark:bg-primary-950/30 rounded-2xl p-5 border border-primary-100 dark:border-primary-900/50">
        ...
        <div className="mt-8 pt-6 border-t border-slate-100 dark:border-slate-800">
          <motion.button
            ...
            className="w-full sm:w-auto sm:min-w-56 px-7 py-3.5 rounded-2xl font-semibold text-sm transition-all
      ...
      {/* 最近面试记录 */}
      <div className="bg-white dark:bg-slate-900 rounded-[28px] shadow-[0_18px_45px_-32px_rgba(24,34,53,0.45)] border border-slate-200/80 dark:border-slate-800 p-6 sm:p-8">
        <div className="flex items-center justify-between mb-5">
          <div>
            <p className="text-xs font-semibold tracking-wide text-slate-400 dark:text-slate-500">YOUR RECENT PRACTICE</p>
            <h2 className="text-xl font-bold text-slate-900 dark:text-white mt-1">最近面试记录</h2>
          </div>
          <Link
            to="/interviews"
            ...
            {recentInterviews.map((item, index) => {
              ...
              return (
                <motion.div
                  ...
                  className="flex items-center gap-4 p-4 rounded-2xl border border-transparent hover:border-slate-200 dark:hover:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors cursor-pointer group"
                >
                  {/* 类型图标 */}
