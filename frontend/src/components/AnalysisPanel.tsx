...
export default function AnalysisPanel({
  ...
  if (isProcessing) {
    ...
    return (
        <div className="rounded-[28px] border border-slate-200/80 bg-white p-12 text-center shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div
  ...
  if (analyzeStatus === 'FAILED' || !isAnalysisValid) {
    return (
        <div className="rounded-[28px] border border-red-100 bg-white p-12 text-center shadow-sm dark:border-red-950/70 dark:bg-slate-900">
          <div
        ...
        {onReanalyze && (
          <motion.button
            ...
            className="mx-auto flex items-center gap-2 rounded-xl bg-primary-600 px-6 py-2.5 font-medium text-white shadow-md shadow-primary-600/20 transition-colors hover:bg-primary-700 disabled:opacity-50"
        ...
        >
          <div className="flex items-center justify-between mb-6">
            ...
            <motion.button
              onClick={onExport}
              disabled={exporting}
              className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-medium text-slate-600 transition-all hover:bg-slate-50 disabled:opacity-50 dark:border-slate-700 dark:bg-slate-950/30 dark:text-slate-300 dark:hover:bg-slate-800"
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
            >
          ...
          <div
              className="rounded-2xl border border-emerald-100 bg-emerald-50/70 p-6 dark:border-emerald-900/70 dark:bg-emerald-950/25">
            <p className="text-lg text-slate-800 dark:text-white leading-relaxed mb-6">
              {analysis.summary || '候选人具备扎实的技术基础，有大型项目架构经验。'}
            </p>

            <div className="grid grid-cols-2 gap-4 mb-4">
              <div className="rounded-xl border border-emerald-100 bg-white/90 p-5 dark:border-emerald-900/60 dark:bg-slate-900/80">
            ...
            {analysis.strengths && analysis.strengths.length > 0 && (
                <div className="rounded-xl border border-emerald-100 bg-white/90 p-4 dark:border-emerald-900/60 dark:bg-slate-900/80">
                  <span
